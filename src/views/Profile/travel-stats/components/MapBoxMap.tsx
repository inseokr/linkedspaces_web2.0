...
export default function MapboxMap({
  ...
  const buildCountryFilter = (iso2List: string[], worldviewValue: string) => {
    ...
    // (선택) worldview: all 이거나 해당 worldviewValue
    // - 데이터가 "all"/"US"/"KR" 같은 문자열일 때 가장 안전
    const worldviewOk: any = [
      "any",
      ["==", "worldview", "all"],
    ...
    if (!iso2List || iso2List.length === 0) {
      return ["all", disputedOk, worldviewOk];
    }

    // iso_3166_1이 리스트에 포함되는지 (필터 문법은 값을 펼쳐 넣음)
    const isoFilter: any = ["in", "iso_3166_1", ...iso2List];

    return ["all", disputedOk, worldviewOk, isoFilter];
  };
  ...
  const focusOnCountry = (iso2: string) => {
    ...
    map.easeTo({ center, zoom: 8, duration: 650 });
  };
